...
const CLICKABLE_SELECTOR = "a, button, [role='menuitem'], [role='link'], [role='button'], [onclick]";

export class ReadOnlyPathNavigator {
  ...
  async open(path: readonly string[]): Promise<ReadOnlyNavigationResult> {
    ...
    try {
      for (let actionCount = 0; actionCount < path.length; actionCount += 1) {
        ...
        if (!await waitForDeeperStep(this.#rootDocument, path, match.index)) return "not-found";
      }
      return "not-found";
    } finally {
      this.#busy = false;
...
function waitForDeeperStep(
  ...
  return new Promise((resolve) => {
    ...
    const check = (): void => {
      if (deepestAvailableMatch(rootDocument, path, afterIndex).kind !== "missing") finish(true);
    };
    for (const entry of accessibleDocuments(rootDocument)) {
      if (entry.document.documentElement) {
        const observer = new MutationObserver(check);
        observer.observe(entry.document.documentElement, { childList: true, subtree: true });
        observers.push(observer);
      }
      for (const frame of entry.document.querySelectorAll("iframe")) {
