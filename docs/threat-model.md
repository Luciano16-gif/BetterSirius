...
- The manifest grants only the two observed HTTP components of Sirius: the `sirius.unimet.edu.ve` portal and its `sappro2.unimet.edu.ve` SAP application frame host. The latter receives only the passive all-frame reader; shell mounting and navigation remain restricted at runtime to `/irj/` on the portal host. HTTPS is intentionally absent until its availability is verified separately and explicitly.
- No academic or financial SAP form is submitted programmatically. Delegation is limited to explicit reads: the two observed grade-query entries, `Oferta Académica`, `Turno de Inscripción`, and `Web de Pagos`. The registration-window adapter reads only its four scheduling values and ignores adjacent identity fields. The Web de Pagos reader retains only four labeled totals, the pending-payment status, and message-count metadata; it never touches payment-method controls. Offer search either copies one user-entered course code into the current visible criterion or uses the visible name-or-code value help. Lookup searches occur only on submit; selection requires an exact visible code/name match and activates the visible offer search once. There is no automatic retry or request-per-keystroke behavior.
...
Registration, add/drop or withdrawal mutations, payment execution, payment-method selection, debt-line selection, password changes/security management, profile, email, document generation, academic-data caching, and any other live-page mutation are intentionally unimplemented. Showing already-recorded withdrawn courses is a read filter, not a withdrawal action. A future change to any mutating area requires a separate threat-model update, sanitized fixtures for every known outcome, and explicit confirmation boundaries before any live validation.
